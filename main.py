...
from src.tests.UI_perm_test import permutation_test, asymptotic_test, gamma_non_asympt_test, RFF_perm_test
...
dependence_type = 'direct_dependence' ### 'independence' or 'direct_dependence' or 'common_confounder'
...
nb_features = 30    ## nb features for the RFF estimation
# if n < 100:
#     nb_shifts = 100
...
def get_res(X, Y):
    ...
    for algo in algo_list:
        ...
        if 'gamma_params' in res:
            res_dict = {'algorithm': algo,
                        'pvalue': res['pval'], 'value':res['test_statistic'],
                        'shape':res['gamma_params']['shape'], 'scale':res['gamma_params']['scale'], 'time':tmp}    
        else :
            res_dict = {'algorithm': algo,
                        'pvalue': res['pval'], 'value':res['test_statistic'],
...
# algo_list = ['RFF_shift_test', 'shift_test','gamma_non_asymptotic', 'asymptotic', 'permutation_test']
algo_list = ['shift_test','random_shift_test', 'RFF_shift_test','asymptotic','permutation_test']
...
def plot_histogram_with_gamma(data_x, data_y, save = False):
    ...
    if 'random_shift_test' in algo_list :
        ...
        # Plot histogram of T_perm_list
        plt.hist(T_H0, density = True, alpha=0.3, color='gold', label='Random shifts Distribution')
        algo_base = 'r_shifts'
    ...
    ############ FIT GAMMA OVER PERMS ##########
    
    
    # shape, loc, scale = pearson3.fit(hsic_data['test_stat_H0'])
    # pdf_gamma_perm = pearson3.pdf(xx, shape, loc, scale)
   ...
   ## plot gamma pdf from permutations 
    
    # plt.plot(xx, pdf_gamma_perm, color = 'gold', label = 'pdf of permutation based gamma')
    ...
    #plt.title(f'{algo_base} Distribution with Gamma Approximation (HSIC Test), n = {n}')
    ...
    plt.legend()
    if save:
        plt.savefig(save_fig_path + f'UI_testing_{data_type}_n{n}_alpha{alpha}.pdf',format = 'pdf')        
    # Show the plot
    plt.show()
...
save_fig_path = current_dir+'/plot/complexity/'
# Assuming data_x and data_y are your input data arrays
i=0

# data_path = path_data + f'/autocorr/{dependence_type}/'
...
##############################################################
######### RUN FOR DIFFERENT AUTO-CORRELATION COEFFS ##########
##############################################################
...
path_results = current_dir+f'/results/ts/power/size{n}/'
# Ensure the directory exists
...
for alpha in autocorr:
    ...
    df_results.to_pickle(path_results + f'{dependence_type}_size_{n}_alpha{alpha}.pkl')
